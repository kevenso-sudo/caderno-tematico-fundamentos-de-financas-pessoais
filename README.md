<Caderno Temático - Fundamentos de Finanças Pessoais></Caderno Temático - Fundamentos de Finanças Pessoais>

Sobre o projeto

Este repositório apresenta um caderno temático de estudos sobre Fundamentos de Finanças Pessoais, desenvolvido a partir da curadoria de fontes, elaboração de perguntas estratégicas e utilização do NotebookLM como ferramenta de apoio à pesquisa e aprendizagem.

O projeto busca documentar não apenas os conhecimentos adquiridos sobre o tema, mas também o processo de investigação, elaboração e refinamento de prompts utilizados durante o estudo.

---

Contexto e objetivos

Tema escolhido

Fundamentos de Finanças Pessoais

A Finanças Pessoais foi escolhida por sua importância para compreender como administrar o próprio dinheiro possa garantir estabilidade econômica, segurança e qualidade de vida, impactando positivamente diversas área do cotodiano.

Além da relevância acadêmica para a área de Administração, o tema permite estabelecer a aplicação de conceitos empresariais, contábeis e financeiros para a tomada de decisão no contexto individual e familiar.

Objetivo geral

Compreender os principais conceitos relacionados à finanças pessoais e analisar como ela influencia na estabilidade econômica e tranquilidade financeira

Objetivos específicos

* Compreender o conceito de finanças pessoais.
* Identificar seus principais elementos.
* Gestão comportamental e consumo consciente. 
* Administrar e mitigar risco.
* Realizar metas de vida.
* Relacionar os conceitos estudados com situações práticas.
* Desenvolver prompts capazes de apoiar futuras revisões sobre o tema.

---

Curadoria de fontes

Foram selecionadas fontes acadêmicas e institucionais relacionadas à finanças pessoais, priorizando materiais com conteúdo conceitual e aplicações práticas.

| # | Fonte                                                                                                                                                                                                   | Tipo                   | Principal contribuição                            |
| - | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ------------------------------------------------- |
| 1 | Amanda Oliveira Gomes e Rafaela Brito da Silva - A contribuição da contabilidade para gestão das finanças pessoais e familiares                                                                         | Artigo científico      | Conceitos e contabilidade                         |
| 2 | Liliane Leopoldino Tôrres e Douglas Willyam Rodrigues Gomes - Educação Financeira e Gestão Inteligente das Finanças                                                                                     | Artigo científico      | Educação financeira e consumo consciente          |
| 3 | Thatiany Seára da Silveira e Azevedo - Finanças Pessoais                                                                                                                                                | Monografia (TCC)       | Conceitos e fatores comportamentais e emocionais  |
| 4 | Beatriz Tavares Silva - Gestão Financeira Pessoal: O Impacto de Decisões Efetivas                                                                                                                       | Monografia (TCC)       | Conceitos e gestão financeira                     |
| 5 | Crislaine de Lima Feitosa, Vera Lúcia Cruz, Mayara dos Santos Silva e Luiz Antônio Félix Júnior - Um estudo sobre a utilização das ferramentas contábeis no controle orçamentário das pessoas físicas   | Artigo científico      | Ferramentas contábeis e orçamento                 |

As informações completas das fontes estão disponíveis em [`02-curadoria-de-fontes/`](./02-curadoria-de-fontes/).

---

Engenharia de Prompts

Durante o estudo foram desenvolvidas e testadas perguntas e prompts para:

* compreender e diferenciar conceitos;
* utilizar ferramentas de controle;
* obter exemplos práticos;
* testar a compreensão do conteúdo;
* revisar conhecimentos;
* identificar lacunas de aprendizagem.

Também foram documentadas as dificuldades encontradas durante o processo e as alterações realizadas nos prompts.

Consulte [`03-engenharia-de-prompts/`](./03-engenharia-de-prompts/).

---

Miniguia de estudo

O resultado final foi consolidado em um miniguia contendo:

* resumos estruturados;
* conceitos fundamentais;
* glossário;
* relações entre conceitos;
* aplicações práticas;
* prompts reutilizáveis para futuras revisões.

Consulte [`04-miniguia-de-estudo/`](./04-miniguia-de-estudo/).

---

##Principais aprendizados

### Sobre o conteúdo

* As finanças pessoais representa padrões compartilhados de valores, crenças e comportamentos.
* O aprendizado financeiro ajuda no desenvolvimento de habilidades e competência para fazer escolhas inteligentes.
* A contabilidade também é para a Pessoa Física.
* Emoções e comportamentos ditam as decisões financeiras.
* Táticas facéis para controlar o dinheiro.

### Sobre o processo de aprendizagem

O uso de IA mostrou-se mais eficiente quando as perguntas apresentavam contexto, objetivo, critérios de resposta e fontes específicas.

Os testes também demonstraram que respostas genéricas podem ser melhoradas por meio do refinamento progressivo dos prompts.

---

Ferramentas utilizadas

* GitHub
* NotebookLM
* Markdown
* ChatGPT
* Google Labs
* Navegador para pesquisa das fontes

---

Estrutura do projeto

```text
01-contexto-e-objetivos/
02-curadoria-de-fontes/
03-engenharia-de-prompts/
04-miniguia-de-estudo/
05-evidencias/
```

---

Autor

Keven Souza de Oliveira 

Projeto desenvolvido como estudo prático de pesquisa, aprendizagem assistida por IA e engenharia de prompts.
